    ...
    graphRef.current.innerHTML = `
      <div class="flex items-center justify-center h-full text-gray-400">
        <div class="text-center">
          ...
          ${nodeCount > 0 ? '<p class="text-xs mt-1 text-green-400">✓ Generated by InfraNodus</p>' : '<p class="text-xs mt-1 text-yellow-400">Using fallback analysis</p>'}
        </div>
      </div>
    `;
  };
            ...
            <div className="space-y-6">
              ...
              {insights?.metadata && (
                <div className="pt-4 border-t border-gray-700">
                  <h4 className="text-sm font-medium text-gray-400 mb-2">Analysis Info</h4>
                  <div className="text-xs text-gray-500 space-y-1">
                    {insights.metadata.questionCount && (
                      <p>Questions: {insights.metadata.questionCount}</p>
                    )}
                    {insights.metadata.usage && (
                      <p>Tokens: {insights.metadata.usage}</p>
                    )}
                    {insights.metadata.timestamp && (
                      <p>Generated: {new Date(parseInt(insights.metadata.timestamp) * 1000).toLocaleString()}</p>
                    )}
                  </div>
                </div>
              )}
